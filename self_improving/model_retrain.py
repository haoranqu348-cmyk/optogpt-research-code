...
import torch
import numpy as np
from optogpt.core.datasets.datasets import PrepareDataAug
from optogpt.core.models.transformer import make_model_I
from optogpt.core.trains.train import LabelSmoothing, run_epoch_I, SimpleLossCompute
...
def retrain_model(model_path, new_train_struct, new_train_spec, new_test_struct, new_test_spec, 
    ...
    a = torch.load(model_path, map_location=device, weights_only=False)
    cfg = a['configs']

    # Helper: handle both dict and SimpleNamespace configs
    def _get(key, default=None):
        if isinstance(cfg, dict):
            return cfg.get(key, default)
        return getattr(cfg, key, default)

    seed = _get('seeds', _get('seed', 42))
    ...
    # Assign to config (works for both dict and namespace)
    if isinstance(cfg, dict):
        cfg['TRAIN_FILE'] = TRAIN_FILE
        cfg['TRAIN_SPEC_FILE'] = TRAIN_SPEC_FILE
        cfg['DEV_FILE'] = DEV_FILE
        cfg['DEV_SPEC_FILE'] = DEV_SPEC_FILE
    else:
        cfg.TRAIN_FILE = TRAIN_FILE
        cfg.TRAIN_SPEC_FILE = TRAIN_SPEC_FILE
        cfg.DEV_FILE = DEV_FILE
    ...
    data = PrepareDataAug(TRAIN_FILE, TRAIN_SPEC_FILE, _get('ratios', 100), DEV_FILE, DEV_SPEC_FILE, _get('batch_size', 1000), _get('spec_type', 'R_T'), 'Inverse', struct_word_dict, struct_index_dict)
    tgt_vocab = len(data.struc_word_dict)
    print(tgt_vocab)
    src_vocab = len(data.dev_spec[0])
    if isinstance(cfg, dict):
        cfg['struc_dim'] = tgt_vocab
        cfg['spec_dim'] = src_vocab
        cfg['struc_index_dict'] = data.struc_index_dict
        cfg['struc_word_dict'] = data.struc_word_dict
    else:
        cfg.struc_dim = tgt_vocab
        cfg.spec_dim = src_vocab
        cfg.struc_index_dict = data.struc_index_dict
    ...
    print(">>>>>>> start train")
    criterion = LabelSmoothing(tgt_vocab, padding_idx=1, smoothing=_get('smoothing', 0.1))
    
    optimizer = FinetuneOpt(_get('d_model', 1024), 5e-5, 1, torch.optim.Adam(model.parameters(), lr=0, betas=(0.9, 0.98), eps=1e-9))
 
    train_I_aug(data, model, criterion, optimizer, cfg, device, epochs, early_stopping_patience, type_T, output_dir)
