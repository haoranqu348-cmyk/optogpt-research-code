...
from optogpt.core.models.transformer import make_model_I, subsequent_mask
from optogpt.core.trains.train import Variable
from generate_dev_data import generate_dev_data
import multiprocessing as mp

from colour import SDS_ILLUMINANTS, MSDS_CMFS

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Incidence angle / polarization — override from run_self_improving.py
THETA = 0   # degrees
POL = "s"   # 's' or 'p'

DATABASE = os.path.join(os.path.dirname(__file__), '..', 'optogpt', 'nk')
...
class Prepare_Augment_Data:
    ...
    def __init__(self, model_path, decoding_method = "Greedy Decode",error_type = "MAE",top_k = 10, top_p = 0.9, kp_num = 50, keep_num = 20):
        ...
        # load nk_dict early (needed for spectrum() calls during decoding)
        self.nk_dict = load_materials(all_mats=mats, wavelengths=wavelengths, DATABASE=DATABASE)
        ...
        if decoding_method == "Greedy Decode":
            ...
            for spec in self.original_spec:
                ...
                for i in range(self.kp_num):
                    ...
                    tmp_error.append(self.calc_single_error(spec, spectrum(tmp_mat[i], int_list, pol=POL, theta=THETA, wavelengths=wavelengths, nk_dict=self.nk_dict)))
    ...
    def _get_cfg(self, cfg, key, default=None):
        """Get config value from dict or SimpleNamespace."""
    ...
    def load_model(self, model_path):
        #load the model
        a = torch.load(model_path, weights_only=False)
        args = a['configs']
        seed = self._get_cfg(args, 'seeds', self._get_cfg(args, 'seed', 42))
        torch.manual_seed(seed)
        np.random.seed(seed)
        model = make_model_I(
                        self._get_cfg(args, 'spec_dim'),
                        self._get_cfg(args, 'struc_dim'),
                        self._get_cfg(args, 'layers', 6),
                        self._get_cfg(args, 'd_model', 1024),
                        self._get_cfg(args, 'd_ff', 512),
                        self._get_cfg(args, 'head_num', 8),
                        self._get_cfg(args, 'dropout', 0.1),
                    ).to(DEVICE)

        model.load_state_dict(a['model_state_dict'], strict=False)
        return model, args
    ...
    def load_data_file(self, model_path):
        ...
        configs = a['configs']
        self.struc_word_dict = self._get_cfg(configs, 'struc_word_dict', {})
        self.struc_index_dict = self._get_cfg(configs, 'struc_index_dict', {})
    ...
    def simulate_spec(self):
        ...
        nk_dict = self.nk_dict
        
        from multiprocessing import Pool
        args_for_starmap = [(mat, thick, POL, THETA, wavelengths, nk_dict, 'Glass_Substrate', 500000) 
