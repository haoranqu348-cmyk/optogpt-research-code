...
from optogpt.core.datasets.datasets import PrepareData
...
def parse_args():
    ...
    parser.add_argument('--theta', type=float, default=0,
                        help='Incidence angle in degrees (e.g., 60)')
    parser.add_argument('--pol', type=str, default='s',
                        choices=['s', 'p'],
                        help='Polarization: s or p')
    parser.add_argument('--augment_only', action='store_true', default=False,
                        help='Only generate augmented data, skip final retraining')
...
def main():
    ...
    # Inject theta/pol for angled incidence (e.g., theta=60, pol='s')
    prepare_aug_data.THETA = args.theta
    prepare_aug_data.POL = args.pol
    data_perturb.THETA = args.theta
    data_perturb.POL = args.pol
    ...
    data = PrepareData(
        args.train_struct_path, 
        args.train_spec_path, 
        _get_cfg(original_args, 'ratios', 100), 
        args.dev_struct_path, 
        args.dev_spec_path, 
        _get_cfg(original_args, 'batch_size', 1000), 
        _get_cfg(original_args, 'spec_type', 'R_T'), 
        'Inverse'
    )
    struct_word_dict = data.struc_word_dict
    struct_index_dict = data.struc_index_dict
    
    # Step 9: Retrain model (skip if --augment_only)
    if args.augment_only:
        print(f"\n9. SKIPPED — --augment_only flag set.")
        print(f"   Augmented data saved to: {os.path.join(args.output_dir, 'augmented_data')}")
        print(f"   Use these files for final combined retraining.")
    else:
