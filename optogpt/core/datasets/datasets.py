...
class Batch:
    ...
    def __init__(self, src, trg, if_inverse = 'Forward', pad=PAD):
...
class PrepareData:
    def __init__(self, train_file, train_spec_file, train_ratio, dev_file, dev_spec_file, BATCH_SIZE=128, spec_type = 'R_T', if_inverse = 'Forward', shuffle_train=False):
        ...
        self.if_inverse = if_inverse
        self.shuffle_train = shuffle_train
        self.train_data = self.splitBatch(self.train_struc, self.train_spec, BATCH_SIZE, if_inverse, shuffle=shuffle_train)
    ...
    def splitBatch(self, struc, spec, batch_size, if_inverse = 'Forward', shuffle=False):
        ...
        When shuffle=True, data indices are randomized before batching
        (NOT just batch order). Call this at the start of each epoch.
        """
        n = len(struc)
        indices = np.arange(n)
        if shuffle:
        ...
        for start in range(0, n, batch_size):
            batch_index = indices[start:min(start + batch_size, n)]
            batch_struc = [struc[idx] for idx in batch_index]
            batch_spec = [spec[idx] for idx in batch_index]

            batch_struc = seq_padding(batch_struc, padding=PAD)
            batches.append(Batch(batch_struc, np.array(batch_spec), if_inverse, pad=PAD))

        return batches


    def reshuffle_train(self):
        """Re-shuffle and re-batch training data (call at start of each epoch)."""
        if self.shuffle_train:
            self.train_data = self.splitBatch(
                self.train_struc, self.train_spec,
                self.BATCH_SIZE, self.if_inverse, shuffle=True)

class PrepareDataAug(PrepareData):
    """
    Same as PrepareData but uses a pre-built vocabulary (struct_word_dict, struct_index_dict)
    instead of building a new one from training data. This is essential for fine-tuning
    where the token IDs must match the pretrained checkpoint exactly.
    """
    def __init__(self, train_file, train_spec_file, train_ratio, dev_file, dev_spec_file,
                 BATCH_SIZE=128, spec_type='R_T', if_inverse='Forward',
                 struct_word_dict=None, struct_index_dict=None,
                 shuffle_train=False):

        # 01. Read the data and tokenize (same as PrepareData)
        self.train_struc, self.train_spec = self.load_data(train_file, train_spec_file, train_ratio)
        self.dev_struc, self.dev_spec = self.load_data(dev_file, dev_spec_file)
        ...
        # 02. Use pre-built dictionary — DO NOT rebuild
        if struct_word_dict is None or struct_index_dict is None:
            raise ValueError("PrepareDataAug requires pre-built struct_word_dict and struct_index_dict")
        self.struc_word_dict = struct_word_dict
        self.struc_index_dict = struct_index_dict
        self.struc_total_words = len(struct_word_dict)

        # 03. word to id by the provided dictionary
        self.train_struc = self.wordToID(self.train_struc, self.struc_word_dict)
        self.dev_struc = self.wordToID(self.dev_struc, self.struc_word_dict)
